            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form action="{{route('statusEvent.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                      <div class="form-group">
                        <label for="formGroupExampleInput2">Nama Status</label>
                        <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="" name="nama_status">
                      </div>
                ...
                <div class="" style="padding:25px">
                    <table class="table table-bordered text-center" id="table-os">
                        <thead class="thead-light">
                            <tr>
                                <th scope="col">{{ __('Nama Status')}}</th>
                        ...
                        <tbody>
                            @foreach($dataStatus as $item)
                              <tr style="text-align: center">
                                <td><a href="{{route('statusEvent.edit', $item->id)}}" title="">{{$item->nama_status}}</a></td>
                                <td>{{$item->created_at}}</td>
                                <td>
                                  <form action="{{route('statusEvent.destroy',$item->id)}}" method="POST">
                                    @csrf
                                    @method('post')
