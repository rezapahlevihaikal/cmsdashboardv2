                ...
                <form class="" action="{{ route('tiktok.update', $dataTiktok->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" class="form-control" id="validationTooltip02" name="dataadd" value="{{$dataTiktok->dataadd}}" required>
                        ...
                        <div class="col">
                            ...
                            <select id="demo_overview_minimal" class="form-control" data-role="select-dropdown" data-profile="minimal" name="website_id" value="" selected="">
                                @foreach ($dataWebsite as $item)
                                    <option value="{{ $item->id }}" {{$dataTiktok->website_id == $item->id  ? 'selected' : ''}}>{{ $item->website_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col">
                            <label for="formGroupExampleInput2">Rank</label>
                            <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="" name="rank" value="{{$dataTiktok->rank}}">
                        </div>
                    </div>
