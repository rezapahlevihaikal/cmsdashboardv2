            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form action="{{route('picEvent.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                      <div class="form-group">
                        <label for="formGroupExampleInput2">Nama PIC</label>
                        <input type="text" class="form-control" id="formGroupExampleInput2" placeholder="" name="nama_pic">
                      </div>
                      <div class="form-group">
                        ...
                        <tbody>
                            @foreach($dataPic as $item)
                              <tr style="text-align: center">
                                <td><a href="{{route('picEvent.edit', $item->id)}}" title="">{{$item->nama_pic}}</a></td>
                                <td>{{$item->divisi}}</td>
                                <td>{{$item->created_at}}</td>
                                <td>
                                  <form action="{{route('picEvent.destroy',$item->id)}}" method="POST">
                                    @csrf
                                    @method('post')
